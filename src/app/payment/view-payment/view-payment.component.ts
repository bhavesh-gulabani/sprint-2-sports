...
export class ViewPaymentComponent implements OnInit {
  ...
  getPayment(id: number) {
    this.paymentService.getPayment(id).subscribe({
      next: payment => {this.payment = payment; console.log(this.payment)},
  ...
  continue() {
    ...
    // this.customer.cart.totalAmount = 0;

    // console.log(this.customer.cart);

    // Update the customer in the backend
    // this.customerService.updateCustomer(this.customer).subscribe({
    //   next: customer => {console.log(customer); this.router.navigate(['/welcome']);},
    //   error: err => this.errorMessage = err
    // });  
